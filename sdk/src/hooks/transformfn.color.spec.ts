...
describe("rgbToHex()", () => {
  const t = rgbToHex();
  it("converts rgb(255, 0, 0) to #ff0000", () => expect(t("rgb(255, 0, 0)")).toBe("#ff0000"));
  it("converts rgb(255, 147, 15) to #ff930f", () => expect(t("rgb(255, 147, 15)")).toBe("#ff930f"));
  it("converts rgba (ignores alpha)", () => expect(t("rgba(255, 0, 0, 0.5)")).toBe("#ff0000"));
  it("converts percentage channels", () => expect(t("rgb(100%, 0%, 0%)")).toBe("#ff0000"));
  it("passes through hex unchanged", () => expect(t("#ff0000")).toBe("#ff0000"));
  it("passes through named colors unchanged", () => expect(t("coral")).toBe("coral"));
  it("passes through hsv unchanged", () =>
    expect(t("hsv(0, 100%, 100%)")).toBe("hsv(0, 100%, 100%)"));
  it("passes through invalid strings unchanged", () => expect(t("banana")).toBe("banana"));
});
...
describe("hsvToHex() / hsbToHex()", () => {
  ...
  it("converts hsb notation", () => expect(t("hsb(0, 100, 100)")).toBe("#ff0000"));
  it("passes through rgb unchanged", () => expect(t("rgb(255, 0, 0)")).toBe("rgb(255, 0, 0)"));
  it("passes through hex unchanged", () => expect(t("#ff0000")).toBe("#ff0000"));
  it("hsbToHex is an alias", () => expect(hsbToHex()("hsb(0, 100, 100)")).toBe("#ff0000"));
});
...
describe("htmlNamedToHex()", () => {
  ...
  it("passes through hex unchanged", () => expect(t("#ff0000")).toBe("#ff0000"));
  it("passes through rgb unchanged", () => expect(t("rgb(255,0,0)")).toBe("rgb(255,0,0)"));
  it("passes through unknown names unchanged", () => expect(t("banana")).toBe("banana"));
});
