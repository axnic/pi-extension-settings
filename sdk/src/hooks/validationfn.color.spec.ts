...
describe("rgbColor()", () => {
  const v = rgbColor();
  it("accepts rgb(int, int, int)", () =>
    expect(v("rgb(255, 0, 0)").valid).toBe(true));
  it("accepts rgba(int, int, int, float)", () =>
    expect(v("rgba(255, 0, 0, 0.5)").valid).toBe(true));
  it("accepts rgb with percentages", () =>
    expect(v("rgb(100%, 0%, 0%)").valid).toBe(true));
  it("accepts rgba with percentages", () =>
    expect(v("rgba(100%, 0%, 0%, 1)").valid).toBe(true));
  it("accepts rgb without spaces", () =>
    expect(v("rgb(255,0,0)").valid).toBe(true));
  it("rejects hex", () => expect(v("#ff0000").valid).toBe(false));
  it("rejects out-of-range channel (256)", () =>
    expect(v("rgb(256, 0, 0)").valid).toBe(false));
  it("rejects out-of-range channel (101%)", () =>
    expect(v("rgb(101%, 0%, 0%)").valid).toBe(false));
  it("rejects float channels", () =>
    expect(v("rgb(1.5, 0, 0)").valid).toBe(false));
  it("rejects named colors", () => expect(v("red").valid).toBe(false));
...
describe("hsvColor() / hsbColor()", () => {
  const v = hsvColor();
  it("accepts hsv(h, s%, v%)", () =>
    expect(v("hsv(30, 94%, 100%)").valid).toBe(true));
  it("accepts hsb(h, s, v) without %", () =>
    expect(v("hsb(0, 100, 100)").valid).toBe(true));
  it("accepts hsv with floats", () =>
    expect(v("hsv(180.5, 50%, 75%)").valid).toBe(true));
  it("is case-insensitive", () =>
    expect(v("HSV(0, 100%, 100%)").valid).toBe(true));
  it("rejects h > 360", () =>
    expect(v("hsv(361, 100%, 100%)").valid).toBe(false));
  it("rejects s > 100%", () =>
    expect(v("hsv(0, 101%, 100%)").valid).toBe(false));
  it("rejects v > 100", () =>
    expect(v("hsv(0, 100%, 101%)").valid).toBe(false));
  it("rejects hex", () => expect(v("#ff0000").valid).toBe(false));
  it("hsbColor is an alias", () =>
    expect(hsbColor()("hsb(0, 100, 100)").valid).toBe(true));
});
...
describe("htmlNamedColor()", () => {
  ...
  it("rejects hex (not a named color)", () =>
    expect(v("#ff0000").valid).toBe(false));
